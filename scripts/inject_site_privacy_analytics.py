...
PRIVACY_ANALYTICS_RE = re.compile(
    r'<script\b[^>]*\bsrc=["\']/?site_privacy_analytics\.js(?:\?[^"\']*)?["\'][^>]*>\s*</script>',
    re.IGNORECASE,
)
...
def remove_privacy_analytics_scripts(html: str) -> str:
    return PRIVACY_ANALYTICS_RE.sub("", html)


def ensure_privacy_analytics_before_footer(html: str) -> tuple[str, bool]:
    footer_script = SITE_FOOTER_RE.search(html)
    if not footer_script:
        return html, False

    existing = PRIVACY_ANALYTICS_RE.search(html)
    if existing and existing.start() < footer_script.start():
        return html, False

    html = remove_privacy_analytics_scripts(html)
    footer_script = SITE_FOOTER_RE.search(html)
    if not footer_script:
        return html, False

    indentation = script_indentation(html, footer_script.start())
    dependency = f'{indentation}<script src="{PRIVACY_ANALYTICS_SRC}"></script>\n'
...
def validate_order(html: str, path: Path) -> None:
    ...
    dependency = PRIVACY_ANALYTICS_RE.search(html)
    if not dependency or dependency.start() > footer_script.start():
        raise SystemExit(f"site_privacy_analytics.js must load before site_footer.js in {path}")
...
def process_site(root: Path) -> int:
    ...
    for path in root.rglob("*.html"):
        ...
        updated, modified = ensure_privacy_analytics_before_footer(html)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ensure site_privacy_analytics.js loads before site_footer.js."
    )
    parser.add_argument(
        "--site-root",
        default="_site",
